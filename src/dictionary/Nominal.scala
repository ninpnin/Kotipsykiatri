...
abstract class Nominal(val sana: String, astevaihtelu: Char) {
    def taivuta(sijamuoto: Int, monikko: Boolean): String
}
...
case class Kotus1(s: String, g: Char) extends Nominal(s,g) { // KOTUS #1 tyyppi VALO
  ...
  def taivuta(sijamuoto: Int, monikko: Boolean) = {

    if (!monikko) {
      val stem = sijamuoto match {
        1 | 2 | 4 | 5 | 7 | 8 | 9 | 11 => heikko
        _ => sana
      }
      val endings = Vector("", "n", "n", "ss"+a, "st"+a, sana.last+"n", "ll"+a, "lt"+a, "lle", "n"+a, "ksi")
      stem + endings(sijamuoto)
    } else {
      val stem = sijamuoto match {
        0 | 4 | 5 | 7 | 8 | 9 | 11 => heikko
        _ => sana
      }
...
case class Kotus2(s: String, g: Char) extends Nominal(s,g) {  //KOTUS TYPE PALVELU
  ...
  def taivuta(sijamuoto: Int, monikko: Boolean): String = if (!monikko) {
      sijamuoto match {
        case 1 | 2 => heikko + "n" // gen. ja akk. ovat samat. K�yt�nn�ss� kaikissa n�in.
        ...
        case 11 => heikko + "ksi"
        case _ => sana        // V��r� indeksi palauttaa nominatiivin
      }
    } else {
      sijamuoto match {
        case 2 => sana + "jen"
...
case class Kotus48(s: String, g: Char) extends Nominal(s,g) {
  ...
  def taivuta(sijamuoto: Int, monikko: Boolean): String = if (!monikko) {
      sijamuoto match {
        case 1 | 2 => heikko + "en" // gen. ja akk. ovat samat. K�yt�nn�ss� kaikissa n�in.
